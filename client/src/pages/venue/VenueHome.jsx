...
import { fetchVenueStatus, getRoleMemory, setRoleMemory, roleUrl, describeMemory } from './venueShared'
import { PinModal, SeatPicker } from './venueWidgets'
...
export default function VenueHome() {
  ...
  const openRole = (mem, pinKind) => {
    const go = (token) => {
      ...
      // v2.5.03: a Head Judge who also holds a judge seat (memory hj + seat)
      // and re-picks the Head Judge tile keeps that seat — the seat is still
      // claimed by this device on the server, so dropping it here would
      // strand the claim and silently lose the judge tab.
      if (mem.role === 'hj') {
        const cur = getRoleMemory()
        if (cur && cur.role === 'hj' && cur.seat) mem = { ...mem, seat: cur.seat }
      }
      setRoleMemory(mem)
