...
router.post('/:runId/scores', async (req, res) => {
  try {
    ...
    // v2.0.02 -- per-judge PINs are no longer enforced anywhere. Tablets are
    // secured by unguessable short-code URLs (access model); venue mode uses
    // the Crew PIN + seat claim (FR-16). Legacy judge rows (e.g. Winfree meet
    // imports) may still carry a pin value -- it is ignored.
