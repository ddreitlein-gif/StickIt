...
const { pairingNumbers: pairingNumbersForRun, roundLabel: roundNameForMatch } = require('../dual/runOrder');
...
function buildBracketPositions(rounds, seedMatches, colW, boxW, colX0, areaTop, areaH, allMain, bracketTotal, boxH) {
  ...
  for (let ci = 1; ci < rounds.length; ci++) {
    ...
    // v2.6.01: anchor on the FIRST round drawn on this page, not the top of
    // the whole bracket — on a page whose first column is the quarterfinals
    // the old anchor collapsed QF positions 1–4 onto semifinal position 1 and
    // the second semifinal (and the connectors into the final) was never drawn.
    const roundOffset   = rounds[0] - round;
...
function keeperRoutes(matches, runoffOption) {
  ...
  const courseOf = (slotCol) => (slotCol === 'registration_id_red' ? 'red' : 'blue');
  const find = (round, pos, small) => matches.find(m =>
    Number(m.bracket_round) === round && Number(m.bracket_position) === pos && !!m.is_small_final === !!small) || null;
  const hasNew58 = !!find(1, 3, true);
  const runoff8  = runoffOption === 'runoff_to_8th';
  const routes = new Map();
  for (const m of matches) {
    const R = Number(m.bracket_round), p = Number(m.bracket_position);
    let win = null, lose = null, place = null;
    if (!m.is_small_final) {
      if (R === 1) {
        place = { win: 1, lose: 2 };
      } else {
        const nm = find(R - 1, Math.ceil(p / 2), false);
        if (nm) win = { id: nm.id, course: courseOf(advancementSlot(p, R)) };
        if (R === 2 && runoffOption !== 'no_runoff') {
    ...
    } else if (R === 2 && hasNew58) {
      const f56 = find(1, 3, true), f78 = find(1, 4, true);
      if (f56) win  = { id: f56.id, course: courseOf(advancementSlot(p, R)) };
      if (f78) lose = { id: f78.id, course: courseOf(consolationSlot(p, R)) };
    } else if (R === 2) {
      place = p === 3 ? { win: 5, lose: 6 } : { win: 7, lose: 8 };   // legacy pre-F-2
    } else if (R === 1) {
      place = p === 2 ? { win: 3, lose: 4 } : p === 3 ? { win: 5, lose: 6 } : { win: 7, lose: 8 };
    }
    routes.set(m.id, { win, lose, place });
  }
  return routes;
}
...
function keeperOrigins(matches, routes) {
  ...
  const get = (id) => { if (!origins.has(id)) origins.set(id, { blue: null, red: null }); return origins.get(id); };
  const main = matches.filter(m => !m.is_small_final);
  const totalRound = main.length ? Math.max(...main.map(m => Number(m.bracket_round))) : 0;
  const seedOf = (m, side) => (side === 'blue' ? (m.blue_dual_seed ?? m.seed_blue) : (m.red_dual_seed ?? m.seed_red)) ?? null;
  for (const m of matches) {
    const o = get(m.id);
    if (!m.is_small_final && Number(m.bracket_round) === totalRound) {
      o.blue = { kind: 'seed', seed: seedOf(m, 'blue') };
      o.red  = m.is_bye ? null : { kind: 'seed', seed: seedOf(m, 'red') };
    }
  }
  for (const m of matches) {
    const r = routes.get(m.id);
    if (!r) continue;
    if (r.win)  get(r.win.id)[r.win.course]   = m.is_bye ? { kind: 'bye', from: m.id, seed: seedOf(m, 'blue') } : { kind: 'winner', from: m.id };
    if (r.lose) get(r.lose.id)[r.lose.course] = m.is_bye ? { kind: 'none', from: m.id } : { kind: 'loser', from: m.id };
  }
  return origins;
}
